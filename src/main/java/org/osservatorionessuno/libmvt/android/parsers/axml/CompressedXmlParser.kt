...
class CompressedXmlParser {
    ...
    private fun parseNamespace(chunkStartIndex: Int, isStartTag: Boolean): Int {
        ...
        if (uri.isNullOrBlank()) {
            LogUtils.w("CompressedXmlParser", "Parsing invalid namespace with uri:$uri")
        } else {
            if (isStartTag) {
    ...
    private fun parseStartTag(chunkStartIndex: Int): Int {
        ...
        LogUtils.d("CompressedXmlParser", "Start tag: ${elementTag.qualifiedName} - $uriIdx - $nameIdx")
        ...
        LogUtils.d("CompressedXmlParser", "Attributes: ${attrs.size}")
        for (attr in attrs) {
            LogUtils.d("CompressedXmlParser", "Attribute: ${attr.identifier.qualifiedName} = ${attr.value}")
        }
